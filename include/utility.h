...
public:
    ...
    ParamServer()
    {
        ...
        //复用数据的内存(引用)，将它们转变为Eigen类型，Eigen默认列优先，因此加上Eigen::RowMajor
    ...
    //首先确定IMU数据是弧度还是角度值，以及加速度是原始单位还是一个0-1的系数乘9.8
    //此函数为利用变换参数将数据从imu坐标系转到了雷达坐标系
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        // rotate gyroscope 角速度
        Eigen::Vector3d gyr(imu_in.angular_velocity.x, imu_in.angular_velocity.y, imu_in.angular_velocity.z);
        gyr = extRot * gyr;
        imu_out.angular_velocity.x = gyr.x();
        imu_out.angular_velocity.y = gyr.y();
        imu_out.angular_velocity.z = gyr.z();
        // rotate roll pitch yaw 旋转 横摇、俯仰、偏航
...
sensor_msgs::PointCloud2 publishCloud(ros::Publisher *thisPub, pcl::PointCloud<PointType>::Ptr thisCloud, ros::Time thisStamp, std::string thisFrame)
{
    ...
    //getNumSubscribers得到订阅该话题的订阅数，决定是否发布此话题
...
void imuRPY2rosRPY(sensor_msgs::Imu *thisImuMsg, T *rosRoll, T *rosPitch, T *rosYaw)
{
    ...
    //将四元数转换为tf::Quaternion四元数形式
    tf::quaternionMsgToTF(thisImuMsg->orientation, orientation);
    //通过四元数构造旋转矩阵，.getRPY用于获取RPY角度
    tf::Matrix3x3(orientation).getRPY(imuRoll, imuPitch, imuYaw);
    //存在double强转float，此处为数值转换，不超过float范围可以强转。
    //但如果直接使用地址转换的话就会出错，因为float指针相当于只取了64位变量的32位，如下：
    //double c = 12.34; float *p; p = (float *) &c;  //出错
    *rosRoll = imuRoll;
    *rosPitch = imuPitch;
    *rosYaw = imuYaw;
}
