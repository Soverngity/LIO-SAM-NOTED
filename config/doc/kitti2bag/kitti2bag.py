...
# IMU数据使用原始数据，即100Hz，而同步后的IMU数据与雷达频率一致为10Hz
def save_imu_data_raw(bag, kitti, imu_frame_id, topic):
    ...
    # 构造imu_data列表，初始化为len(imu_filenames)个[None]
    ...
    for i, imu_file in enumerate(imu_filenames):
        ...
        for line in imu_data_file:
            # 如何只是一个换行符就跳过，每个文件包括一行信息还有一个额外的换行符
            if len(line) == 1:
                continue
    ...
    for timestamp, data in zip(imu_datetimes, imu_data):
        ...
        q = tf.transformations.quaternion_from_euler(roll, pitch, yaw) # 转换为四元数表示
        imu = Imu()
        imu.header.frame_id = imu_frame_id
        ...
        # 发布Topic名为/imu_raw
        bag.write(topic, imu, t=imu.header.stamp)
        # 发布Topic名为/imu_correct，for LIO-SAM GPS
...
def save_dynamic_tf(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
            ...
            # T_w_imu:imu到世界坐标的转换，通过GPS经纬度得到
...
def save_velo_data(bag, kitti, velo_frame_id, topic):
    ...
    count = 0
    # Tqdm 是一个快速，可扩展的Python进度条，可封装任意的迭代器 tqdm(iterator)
    for dt, filename in tqdm(iterable, total=len(velo_filenames)):
        ...
        # get ring channel
        depth = np.linalg.norm(scan, 2, axis=1) # 根据sqrt(x^2+y^2+z^2)得到距离
        ...
        proj_y *= 64  # in [0.0, H]
        proj_y = np.floor(proj_y) # np.floor向下取整
        ...
        # pcl2是一帧点云，数据存储在data字段，fields类似拓展字段
...
if __name__ == "__main__":
    ...
    # cv_bridge将opencv Mat和ROS图像形式进行联系转换
    bridge = CvBridge()
    # ROS包的压缩格式（NONE、BZ2、LZ4）
